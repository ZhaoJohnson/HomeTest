...
namespace homeTest
{
    ...
    {
        //private static ILog //_logger=new LogWithLog4net();
                ...
                foreach (DataColumn column in dtSource.Columns)
                {
                    ...
                    switch (column.DataType.ToString())
                    {
                        ...
                        case "System.DateTime": //日期类型
                            DateTime dateV;
                            DateTime.TryParse(drValue, out dateV);
                            newCell.SetCellValue(dateV.ToString("MMddyyyy"));
        ...
        #region 从DataTable转为Ilist
        public static IList<T> ConvertToModel<T>(DataTable dt) where T : class, new()
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                foreach (PropertyInfo pi in propertys)
                {
                    ...
                    //var modelMapAttribute = pi.GetCustomAttribute(typeof(ModelMapAttribute), false) as ModelMapAttribute;

                    //if (modelMapAttribute != null)
                    //{
                    //    value = dr[modelMapAttribute.ColIndex];
                    //}
                    //else 
                    if (dt.Columns.Contains(tempName.ToLower()))
                    {
                        value = dr[tempName];
                    }
                    ...
                    else
                    {
                        ...
                        if (pi.PropertyType.GenericTypeArguments.Length > 0)
                            piType = pi.PropertyType.GenericTypeArguments[0].Name;
                        else
                            piType = pi.PropertyType.Name;
                        ...
                        switch (piType)
                        {
                            ...
                            case "DateTime":
                                ...
                                break;
                            case "Decimal":
                                pi.SetValue(t, Convert.ToDecimal(value.ToString()), null);
                                break;
                            case "Single":
                                pi.SetValue(t, float.Parse(value.ToString()), null);
                                break;
                            case "DateTimeOffset":
                                pi.SetValue(t, DateTimeOffset.Parse(value.ToString()), null);
                                break;
                            case "Double":
        ...
        /// <param name="sheetName">表</param>
        /// <param name="HeadRow">行头</param>
        /// <returns></returns>
        public static DataTable ImportExceltoDt(string strFileName,string sheetName,int HeadRow)
        {
            DataTable dt = new DataTable();
            IWorkbook wb;
            using (FileStream file = new FileStream(strFileName, FileMode.Open, FileAccess.Read))
            ...
            dt = ImportDt(sheet, HeadRow, true);
            return dt;
        }
        ...
        #region Using One IWorkbook Doing
        public static IWorkbook ImportExceltoWorkBook(Stream stream)
        {
            try
            {
                using (var ms = new MemoryStream())
        ...
        public static ISheet ReadSheetByIWorkBook(IWorkbook wb, int sheetIndex)
        {
            ISheet sheet = wb.GetSheetAt(sheetIndex);
            return sheet;
        }

        public static int ReadSheetRowIndexByIWorkBook(IWorkbook wb, int sheetIndex, string firstUpperColName, int colIndex = 0)
        {
            try
            {
                ISheet sheet = wb.GetSheetAt(sheetIndex);
                ...
                while (true)
                {
                    ...
                    if (row != null)
                    {
                        ...
                        if (cell != null)
                        {
                            if (cell.CellType == CellType.String && cell.StringCellValue?.Trim().ToUpper() == firstUpperColName?.ToUpper().Trim())
                            {
                                return index;
                            }
                        }
                    ...
                    //if (index > 100)
                    //{
                    //    throw new Exception($"can not find header row index with '{firstUpperColName}'");
                    //}
                    index++;
                }
            }
        ...
        public static string ReadCellByIWorkBook(IWorkbook wb, int sheetIndex, int rowIndex, int colIndex)
        {
        ...
        public static DataTable DataTableByIworkBook(IWorkbook wb, int sheetIndex, int rowIndex)
        {
            DataTable dt = new DataTable();
            var sheet = ReadSheetByIWorkBook(wb, sheetIndex);
            dt = ImportDt(sheet, rowIndex, true);
            return dt;
        }
        #endregion

        public static DataTable ImportExceltoDt(Stream stream, int sheetIndex, int HeaderRowIndex)
        {
            try
            {
                using (var ms = new MemoryStream())
        ...
        public static DataTable ImportExceltoDt(Stream stream, int sheetIndex, int HeaderRowIndex, bool excel03)
        {
            try
            {
                using (var ms = new MemoryStream())
        ...
        public static int GetHeaderRowIndex(Stream stream, string sheetName, string firstUpperColName)
        {
            using (var ms = new MemoryStream())
            {
                stream.Position = 0;
        ...
        public static int GetHeaderRowIndex(Stream stream, int sheetIndex, string firstUpperColName)
        {
            using (var ms = new MemoryStream())
            {
                stream.Position = 0;
        ...
        public static string GetSheetOneCell(Stream stream, int sheetIndex, int rowIndex, int colIndex)
        {
            string result = string.Empty;
            try
            {
